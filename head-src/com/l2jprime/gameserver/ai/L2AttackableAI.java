...
import com.l2jprime.gameserver.datatables.sql.TerritoryTable;
...
import com.l2jprime.gameserver.model.actor.instance.L2FestivalMonsterInstance;
import com.l2jprime.gameserver.model.actor.instance.L2FolkInstance;
import com.l2jprime.gameserver.model.actor.instance.L2FriendlyMobInstance;
...
import com.l2jprime.gameserver.model.actor.instance.L2PenaltyMonsterInstance;
...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(final L2Character target)
	{
		...
		if (target.isInvul())
		{
			...
			if (((target instanceof L2PcInstance) && ((L2PcInstance) target).isGM()) || ((target instanceof L2Summon) && ((L2Summon) target).getOwner().isGM()))
			{
				return false;
			}
		}
		...
		if (target instanceof L2PcInstance)
		{
			...
			if ((((L2PcInstance) target).isGM() && ((L2PcInstance) target).getAccessLevel().canTakeAggro()) || (!(me instanceof L2RaidBossInstance) && ((L2PcInstance) target).isSilentMoving()))
			{
				return false;
			}
		...
		else
		{
			...
			if ((target instanceof L2NpcInstance) || L2Character.isInsidePeaceZone(me, target))
			{
				return false;
			}
	...
	private void thinkActive()
	{
		...
		if ((_actor instanceof L2FestivalMonsterInstance) || !npc.canReturnToSpawnPoint())
		{
			return;
		}
