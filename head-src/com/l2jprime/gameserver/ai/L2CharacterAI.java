 ...
 */
package com.l2jprime.gameserver.ai;
...
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_CAST;
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_FOLLOW;
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_IDLE;
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_INTERACT;
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_MOVE_TO;
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_PICK_UP;
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_REST;

import com.l2jprime.gameserver.ai.L2PlayerAI.IntentionCommand;
...
import com.l2jprime.gameserver.network.serverpackets.AutoAttackStop;
...
public class L2CharacterAI extends AbstractAI
{
	...
	{
		if ((target == null) || (getIntention() == AI_INTENTION_REST) || _actor.isAllSkillsDisabled() || _actor.isAfraid())
		{
	...
	{
		if (((getIntention() == AI_INTENTION_REST) && skill.isMagic()) || (_actor.isAllSkillsDisabled() && !skill.isPotion()))
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the L2PcInstance actor
			clientActionFailed();
			return;
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jprime.gameserver.ai.AbstractAI#onIntentionMoveToInABoat(com.l2jprime.gameserver.model.L2CharPosition, com.l2jprime.gameserver.model.L2CharPosition)
	 */
	@Override
	protected void onIntentionMoveToInABoat(final L2CharPosition destination, final L2CharPosition origin)
	{
		if ((getIntention() == AI_INTENTION_REST) || _actor.isAllSkillsDisabled())
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the L2PcInstance actor
			clientActionFailed();
			return;
	...
	{
		if ((getIntention() == AI_INTENTION_REST) || _actor.isAllSkillsDisabled() || _actor.isImobilised() || _actor.isRooted())
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the L2PcInstance actor
			clientActionFailed();
			return;
			...
			if (follow != null)
			{
				...
				if ((getAttackTarget() != null) && (_actor instanceof L2PlayableInstance) && (target instanceof L2PlayableInstance))
				{
					if (getAttackTarget() == follow)
					{
						...
						final boolean isGM = _actor instanceof L2PcInstance ? ((L2PcInstance) _actor).isGM() : false;
	...
	/**
	 * @see com.l2jprime.gameserver.ai.AbstractAI#onIntentionActive()
	 */
	@Override
	 ...
	 */
	public synchronized void set_skill(final L2Skill _skill)
	{
		this._skill = _skill;
	}
