...
public class L2BoxInstance extends L2NpcInstance
{
	...
	{/*
		 ...
		 * statement.setInt(2, getNpcId()); statement.setString(3, drawer); statement.setInt(4, item.getItemId()); statement.setString(5, item.getItem().getName()); statement.setInt(6, item.getCount()); statement.setInt(7, item.getEnchantLevel()); statement.execute(); DatabaseUtils.close(statement);
		 * } else { PreparedStatement statement = con.prepareStatement("UPDATE boxes SET count=? WHERE id=?"); statement.setInt(1, foundCount + item.getCount()); statement.setInt(2, foundId); statement.execute(); DatabaseUtils.close(statement); } } catch (Exception e) {
