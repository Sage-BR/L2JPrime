...
import static com.l2jprime.gameserver.ai.CtrlIntention.AI_INTENTION_ACTIVE;
...
import com.l2jprime.gameserver.datatables.MobGroupTable;
...
import com.l2jprime.gameserver.managers.CustomNpcInstanceManager;
...
import com.l2jprime.gameserver.model.L2DropCategory;
import com.l2jprime.gameserver.model.L2DropData;
...
import com.l2jprime.gameserver.model.actor.knownlist.NpcKnownList;
import com.l2jprime.gameserver.model.actor.stat.NpcStat;
import com.l2jprime.gameserver.model.actor.status.NpcStatus;
...
import com.l2jprime.gameserver.model.entity.event.Lottery;
...
import com.l2jprime.gameserver.model.scripts.L2RBManager;
...
import com.l2jprime.gameserver.network.serverpackets.ExShowVariationCancelWindow;
import com.l2jprime.gameserver.network.serverpackets.ExShowVariationMakeWindow;
...
import com.l2jprime.gameserver.powerpak.Buffer.L2BufferInstance;
import com.l2jprime.gameserver.skills.Stats;
import com.l2jprime.gameserver.taskmanager.DecayTaskManager;
import com.l2jprime.gameserver.templates.L2HelperBuff;
...
public class L2NpcInstance extends L2Character
{
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jprime.gameserver.model.L2Character#getKnownList()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jprime.gameserver.model.L2Character#getStat()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jprime.gameserver.model.L2Character#getStatus()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jprime.gameserver.model.L2Object#isAttackable()
	 */
	@Override
	 ...
	 */
	/*
	 * public void showQuestWindow(L2PcInstance player, String questId) { String content; Quest q = QuestManager.getInstance().getQuest(questId); if (q == null) { // No quests found content =
	 * "<html><body>You are either not on a quest that involves this NPC, or you don't meet this NPC's minimum quest requirements.</body></html>"; } else { if(player.getWeightPenalty() >= 3 && q.getQuestIntId() >= 1 && q.getQuestIntId() < 1000) { player.sendPacket(new
	 * SystemMessage(SystemMessageId.INVENTORY_LESS_THAN_80_PERCENT)); return; } //FileInputStream fis = null; // Get the state of the selected quest QuestState qs = player.getQuestState(questId); if(qs != null) { // If the quest is alreday started, no need to show a window
	 ...
	 * HtmCache.getInstance().getHtm(path); if(Config.DEBUG) { if(content != null) { LOGGER.fine("Showing quest window for quest " + questId + " html path: " + path); } else { LOGGER.fine("File not exists for quest " + questId + " html path: " + path); } } qs = null; } // Send a Server->Client
	 * packet NpcHtmlMessage to the L2PcInstance in order to display the message of the L2NpcInstance if(content != null) { insertObjectIdAndShowChatWindow(player, content); content = null; } // Send a Server->Client ActionFailed to the L2PcInstance in order to avoid that the client wait another
